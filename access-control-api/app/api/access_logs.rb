module API
  class AccessLogs < Grape::API
    ...
    resource :access do
      ...
      post :entry do
        ...
        unless ticket
          ...
          ticket = Ticket.create!(
            ...
            category: ticket_data['category'].downcase  # Ensure category is "vip" or "base"
          )
        end
      ...
      post :exit do
        # Look up ticket using a string external_id for consistency.
        ticket = Ticket.find_by(external_id: params[:ticket_id].to_s)
        error!('Ticket not found', 404) unless ticket

        last_log = ticket.access_logs.order(check_time: :desc).first
        # Only allow an exit if the last log indicates an open ("entry") session.
    ...
    # GET endpoint for filtering logs exists under /api/access_logs
